...
```
1. class apex.parallel.DistributedDataParallel(module, message_size=10000000, delay_allreduce=False, shared_param=None, allreduce_trigger_params=None, retain_allreduce_buffers=False, allreduce_always_fp32=False, num_allreduce_streams=1, allreduce_communicators=None, gradient_average=True, gradient_predivide_factor=1.0, gradient_average_split_factor=None, prof=False)
```

```
2. class apex.parallel.Reducer(module_or_grads_list)
```

3. 普通的BN是在每张卡上根据每张卡的batchsize完成的，而SyncBatchNorm结合DistributedDataParallel使用，可以有效的增加batchsize的大小，以更好的突出BN的作用（可以有效的缓解内部协方差偏移，加速网络收敛，在一定程度上降低过拟合风险等）。
